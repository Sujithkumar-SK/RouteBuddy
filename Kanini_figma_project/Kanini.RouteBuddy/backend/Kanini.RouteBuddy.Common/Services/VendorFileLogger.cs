...
public static class VendorFileLogger
{
    ...
    private static void WriteLog(string level, string message, params object[] args)
    {
        try
        {
            ...
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [{level}] VENDOR: {string.Format(message, args)}";
            var filePath = Path.Combine(LogsDirectory, $"vendor-{DateTime.Now:yyyy-MM-dd}.txt");
    ...
    private static void WriteToErrorFile(string message)
    {
        try
        {
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [ERROR] VENDOR: {message}";
            var filePath = Path.Combine(LogsDirectory, $"vendor-errors-{DateTime.Now:yyyy-MM-dd}.txt");
            File.AppendAllText(filePath, logEntry + Environment.NewLine);
        }
        catch
        {
