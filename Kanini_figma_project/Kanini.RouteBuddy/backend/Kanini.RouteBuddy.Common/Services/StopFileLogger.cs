...
public static class StopFileLogger
{
    ...
    private static void WriteLog(string level, string message, params object[] args)
    {
        try
        {
            ...
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [{level}] STOP: {string.Format(message, args)}";
            var filePath = Path.Combine(LogsDirectory, $"stop-{DateTime.Now:yyyy-MM-dd}.txt");
    ...
    private static void WriteToErrorFile(string message)
    {
        try
        {
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [ERROR] STOP: {message}";
            var filePath = Path.Combine(LogsDirectory, $"stop-errors-{DateTime.Now:yyyy-MM-dd}.txt");
            File.AppendAllText(filePath, logEntry + Environment.NewLine);
        }
        catch { }
    }
