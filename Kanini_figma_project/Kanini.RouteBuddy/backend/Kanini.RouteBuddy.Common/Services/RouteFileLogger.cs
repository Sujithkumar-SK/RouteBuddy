...
public static class RouteFileLogger
{
    ...
    private static void WriteLog(string level, string message, params object[] args)
    {
        try
        {
            ...
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [{level}] ROUTE: {string.Format(message, args)}";
            var filePath = Path.Combine(LogsDirectory, $"route-{DateTime.Now:yyyy-MM-dd}.txt");
    ...
    private static void WriteToErrorFile(string message)
    {
        try
        {
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [ERROR] ROUTE: {message}";
            var filePath = Path.Combine(LogsDirectory, $"route-errors-{DateTime.Now:yyyy-MM-dd}.txt");
            File.AppendAllText(filePath, logEntry + Environment.NewLine);
        }
        catch { }
    }
