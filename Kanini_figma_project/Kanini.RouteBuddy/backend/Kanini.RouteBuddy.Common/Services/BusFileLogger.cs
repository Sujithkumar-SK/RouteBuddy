...
public static class BusFileLogger
{
    ...
    private static void WriteLog(string level, string message, params object[] args)
    {
        try
        {
            ...
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [{level}] BUS: {string.Format(message, args)}";
            var filePath = Path.Combine(LogsDirectory, $"bus-{DateTime.Now:yyyy-MM-dd}.txt");
    ...
    private static void WriteToErrorFile(string message)
    {
        try
        {
            var logEntry = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [ERROR] BUS: {message}";
            var filePath = Path.Combine(LogsDirectory, $"bus-errors-{DateTime.Now:yyyy-MM-dd}.txt");
            File.AppendAllText(filePath, logEntry + Environment.NewLine);
        }
        catch { }
    }
