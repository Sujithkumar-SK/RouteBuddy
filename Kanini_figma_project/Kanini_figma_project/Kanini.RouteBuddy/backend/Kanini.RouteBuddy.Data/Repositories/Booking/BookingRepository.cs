...
namespace Kanini.RouteBuddy.Data.Repositories.Booking
{
    public class BookingRepository : IBookingRepository
    {
            ...
            try
            {
                ...
                while (await reader.ReadAsync())
                {
                    bookings.Add(new Entities.Booking
                    {
                        ...
                        PNRNo = reader.IsDBNull("PNRNo") ? "" : reader.GetString("PNRNo"),
                        TotalSeats = reader.IsDBNull("TotalSeats") ? 0 : reader.GetInt32("TotalSeats"),
                        TotalAmount = reader.IsDBNull("TotalAmount") ? 0 : reader.GetDecimal("TotalAmount"),
                        TravelDate = reader.IsDBNull("TravelDate") ? DateTime.MinValue : reader.GetDateTime("TravelDate"),
                        Status = (BookingStatus)(reader.IsDBNull("Status") ? 1 : reader.GetInt32("Status")),
                        BookedAt = reader.IsDBNull("BookedAt") ? DateTime.MinValue : reader.GetDateTime("BookedAt"),
                        Customer = new Entities.Customer
                        {
                            FirstName = reader.IsDBNull("CustomerName") ? "" : reader.GetString("CustomerName").Split(' ')[0],
                            LastName = reader.IsDBNull("CustomerName") ? "" : (reader.GetString("CustomerName").Contains(' ') ? reader.GetString("CustomerName").Split(' ')[1] : ""),
                            User = new Entities.User
                            {
                                Email = reader.IsDBNull("CustomerEmail") ? "" : reader.GetString("CustomerEmail"),
                                Phone = reader.IsDBNull("CustomerPhone") ? "" : reader.GetString("CustomerPhone")
                            }
                        },
                        Payment = new Entities.Payment
                        {
                            PaymentStatus = (PaymentStatus)(reader.IsDBNull("PaymentStatus") ? 1 : reader.GetInt32("PaymentStatus"))
                        },
        ...
        public async Task<(int Pending, int Confirmed, int Cancelled, int Total, decimal Revenue, int SuccessfulPayments, int PendingPayments)> GetBookingStatusSummaryAsync()
        {
            ...
            try
            {
                ...
                if (await reader.ReadAsync())
                {
                    ...
                    var result = (
                        ...
                        reader.GetInt32("SuccessfulPayments"),
                        reader.GetInt32("PendingPayments")
                    );
