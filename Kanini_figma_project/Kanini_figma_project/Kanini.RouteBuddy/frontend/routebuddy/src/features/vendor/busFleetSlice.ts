...
export const fetchSeatLayoutTemplates = createAsyncThunk(
  ...
  async (busType?: number, { rejectWithValue }) => {
    try {
      const response = await busFleetAPI.getSeatLayoutTemplates(busType);
      return response;
    } catch (error: any) {
    ...
    builder
      ...
      .addCase(createBus.fulfilled, (state, action) => {
        ...
        if (action.payload.data) {
          state.buses.unshift(action.payload.data);
        }
      })
      ...
      .addCase(fetchBusPhotos.fulfilled, (state, action) => {
        state.busPhotos = action.payload.data || [];
      })
      ...
      .addCase(fetchSeatLayoutTemplates.fulfilled, (state, action) => {
        state.seatLayoutTemplates = action.payload.data || [];
      });
  },
});
