...
export const busFleetAPI = {
  ...
  getSeatLayoutTemplates: async (busType?: number) => {
    const params = busType ? { busType } : {};
    const response = await api.get('/admin/AdminSeatLayout/seat-layout-templates', { params });
    return response.data;
  },
